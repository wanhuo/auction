...
use \Workerman\Config\Mongo as MConfig;
...
//        }
        try {
            $this->mongo = new \MongoClient("mongodb://{$config['host']}:{$config['port']}", array('connect'=>$connect));
        }
    ...
    function getInstance($mongo_server, $flag=array())
    {
        ...
        {
            $mongo = new MyMongo($mongo_server);
    ...
    function insert($table_name, $record)
    {
        $dbname = $this->curr_db_name;
        try {
            $this->mongo->$dbname->$table_name->insert($record, array('m'=>true));
            return true;
        }
        catch (\MongoCursorException $e)
        {
    ...
    function update($table_name, $condition, $newdata, $options=array())
    {
        ...
        {
            $this->error = $e->getMessage();
            file_put_contents('/var/tmp/mongo.err', $this->error,FILE_APPEND);
            return false;
        }
    }
